...
<Cases>
  <Case span={2} type="good" title="aria-labelledby属性を使用する">
    <div slot="figure">
      ```html
      <form>
    ...
    コントロールに`aria-labelledby`属性を付与し、ラベルの要素（不可視でも構わない）の`id`を指定します。
  </Case>
</Cases>
...
<Cases>
  <Case span={2} type="good" title="aria-label属性を使用する">
    <div slot="figure">
      ```html
      <fieldset>
        <legend>郵便番号</legend>
    ...
    `aria-label`属性を使うとコントロールに名前を直接指定できます。
  </Case>
</Cases>
